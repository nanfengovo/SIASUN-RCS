...
namespace SIASUN.RCS.Tasks.Workflow
{
    ...
    public interface ITaskWorkflowEngine
    {
        ...
        /// </summary>
        /// <param name="task">任务契约实例</param>
        /// <param name="workflowKey">工作流类型标识（默认为 "Default"）</param>
        /// <param name="cancellationToken">取消令牌</param>
        /// <returns>是否已成功推进或结束</returns>
        Task<StepExecutionResult> ExecuteCurrentStepAsync(IWorkflowTask task, string workflowKey = "Default", CancellationToken cancellationToken = default);
        ...
        Task<StepExecutionResult> ResumeBySignalAsync(IWorkflowTask task, string signalEvent, object? payload = null, string workflowKey = "Default", CancellationToken cancellationToken = default);
        ...
        Task<bool> RollbackToStepAsync(IWorkflowTask task, int targetStepIndex, string reason, string workflowKey = "Default", CancellationToken cancellationToken = default);
    }
}
